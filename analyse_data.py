...
def metrics_per_epoch(data, model, class_label):
    ...
    with open(
        ...
        combined = combined_metric.compute(average="weighted")
        metrics = {
            "accuracy": floor(accuracy["accuracy"] * 100) / 100.0,
            "precision": floor(combined["precision"] * 100) / 100.0,
            "recall": floor(combined["recall"] * 100) / 100.0,
            "f1": floor(combined["f1"] * 100) / 100.0,
        }
